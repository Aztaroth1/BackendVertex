...
admin.initializeApp({
  ...
  databaseURL: 'https://transferenciasvertex-default-rtdb.firebaseio.com/',
});
...
app.post('/recargas', async (req, res) => {
  ...
  try {
    // Obtener referencia al cliente con el número de cuenta especificado
    ...
    let clienteKey;
    clienteSnapshot.forEach(snapshot => {
      clienteKey = snapshot.key;
    });

    // Registrar la recarga en la colección "recargas"
    const newRecargaRef = db.ref('recargas').push();
    await newRecargaRef.set({
      numeroCuenta,
      monto,
      saldo,
      fecha,
    ...
    // Actualizar el saldo del cliente en la colección "clientes"
    await db.ref(`clientes/${clienteKey}`).update({ saldo });

    res.status(200).json({ message: 'Recarga registrada y saldo actualizado con éxito' });
  } catch (error) {
